...
class DatabaseService:
    ...
    @staticmethod
    async def get_tasks_by_user(session: AsyncSession, user_id: UUID, completed: Optional[bool] = None) -> List[Task]:
    ...
    @staticmethod
    async def update_task(session: AsyncSession, task_id: UUID, user_id: UUID, task_update: TaskUpdate) -> Optional[Task]:
        """Update a task for a specific user."""
        db_task = await DatabaseService.get_task_by_id(session, task_id, user_id)
        if not db_task:
            return None
    ...
    @staticmethod
    async def create_conversation(session: AsyncSession, user_id: UUID, title: str) -> Conversation:
    ...
    @staticmethod
    async def get_conversations_by_user(session: AsyncSession, user_id: UUID) -> List[Conversation]:
    ...
    @staticmethod
    async def update_conversation(session: AsyncSession, conversation_id: UUID, user_id: UUID, title: str) -> Optional[Conversation]:
        """Update a conversation for a specific user."""
        db_conversation = await DatabaseService.get_conversation_by_id(session, conversation_id, user_id)
    ...
    @staticmethod
    async def create_message(session: AsyncSession, conversation_id: UUID, role: str, content: str, metadata_json: Optional[str] = None) -> Message:
    ...
    @staticmethod
    async def get_messages_by_conversation(session: AsyncSession, conversation_id: UUID) -> List[Message]:
    ...
    @staticmethod
    async def get_latest_messages(session: AsyncSession, conversation_id: UUID, limit: int = 10) -> List[Message]:
